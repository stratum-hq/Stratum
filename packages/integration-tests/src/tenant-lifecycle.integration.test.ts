...
describe("Tenant Lifecycle (integration)", () => {
  ...
  it("creates a root tenant with correct ancestry", async () => {
    const tenant = await stratum.createTenant(
      tenantInput({ name: "Acme Corp", slug: "acme" }),
    );
  ...
  it("creates a child tenant with parent ancestry", async () => {
    const root = await stratum.createTenant(
      tenantInput({ name: "Root", slug: "root" }),
    );
    const child = await stratum.createTenant(
      tenantInput({ name: "Child", slug: "child", parent_id: root.id }),
    );
  ...
  it("creates a 3-level hierarchy", async () => {
    const root = await stratum.createTenant(
      tenantInput({ name: "Root", slug: "root_a" }),
    );
    const mid = await stratum.createTenant(
      tenantInput({ name: "Mid", slug: "mid_a", parent_id: root.id }),
    );
    const leaf = await stratum.createTenant(
      tenantInput({ name: "Leaf", slug: "leaf_a", parent_id: mid.id }),
    );
  ...
  it("lists tenants", async () => {
    await stratum.createTenant(tenantInput({ name: "T1", slug: "t1" }));
    await stratum.createTenant(tenantInput({ name: "T2", slug: "t2" }));
  ...
  it("gets a tenant by id", async () => {
    const created = await stratum.createTenant(
      tenantInput({ name: "Find Me", slug: "findme" }),
    );
  ...
  it("rejects duplicate slugs", async () => {
    await stratum.createTenant(
      tenantInput({ name: "First", slug: "unique_slug" }),
    );
    await expect(
      stratum.createTenant(tenantInput({ name: "Second", slug: "unique_slug" })),
    ).rejects.toThrow();
  });
});
