...
describe("ABAC evaluation + hierarchy (integration)", () => {
  ...
  it("lets deny override allow even when the allow has higher priority", async () => {
    const t = await stratum.createTenant({
      name: "T",
      slug: uniqueSlug("aden"),
    });
    await stratum.createAbacPolicy(
      t.id,
      policy({ name: "allow_all", effect: "allow", priority: 100 }),
    );
    await stratum.createAbacPolicy(
      t.id,
      policy({ name: "deny_all", effect: "deny", priority: 1 }),
    );
  ...
  it("evaluates a descendant against an ancestor's inherited policy", async () => {
    const root = await stratum.createTenant({
      name: "Root",
      slug: uniqueSlug("ainr"),
    });
    const child = await stratum.createTenant({
      name: "Child",
      slug: uniqueSlug("ainc"),
      parent_id: root.id,
    });
  ...
  it("blocks a descendant from creating a policy an ancestor LOCKED (same composite)", async () => {
    const root = await stratum.createTenant({
      name: "Root",
      slug: uniqueSlug("alkr"),
    });
    const child = await stratum.createTenant({
      name: "Child",
      slug: uniqueSlug("alkc"),
      parent_id: root.id,
    });
    await stratum.createAbacPolicy(
      root.id,
      policy({ name: "gate", mode: "LOCKED" }),
    );
  ...
  it("cascades deleteAbacPolicy to descendant copies of the same composite only", async () => {
    const root = await stratum.createTenant({
      name: "Root",
      slug: uniqueSlug("acar"),
    });
    const child = await stratum.createTenant({
      name: "Child",
      slug: uniqueSlug("acac"),
      parent_id: root.id,
    });

    const rootGate = await stratum.createAbacPolicy(
      root.id,
      policy({ name: "gate" }),
    );
    ...
    const remaining = (await stratum.resolveAbacPolicies(child.id)).map(
      (r) => r.policy.name,
    );
