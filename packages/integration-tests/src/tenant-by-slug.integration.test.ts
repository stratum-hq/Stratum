...
describe("getTenantBySlug (integration)", () => {
  ...
  it("resolves an existing slug to the right tenant", async () => {
    ...
    const created = await stratum.createTenant(
      tenantInput({ name: "Acme Corp", slug }),
    );
  ...
  it("resolves the correct tenant when several exist", async () => {
    ...
    const a = await stratum.createTenant(tenantInput({ name: "A", slug: slugA }));
    await stratum.createTenant(tenantInput({ name: "B", slug: slugB }));
  ...
  it("throws TenantArchivedError for an archived slug by default, like getTenant", async () => {
    ...
    const t = await stratum.createTenant(tenantInput({ name: "Archived", slug }));
    await stratum.archiveTenant(t.id);

    await expect(stratum.getTenantBySlug(slug)).rejects.toThrow(TenantArchivedError);
  ...
  it("throws TenantSuspendedError for a suspended slug by default, like getTenant", async () => {
    ...
    const t = await stratum.createTenant(tenantInput({ name: "Suspended", slug }));
    await stratum.suspendTenant(t.id);

    await expect(stratum.getTenantBySlug(slug)).rejects.toThrow(TenantSuspendedError);
