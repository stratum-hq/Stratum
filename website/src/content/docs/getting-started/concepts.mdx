...
|-----------|-------------|
| `getAncestors(id)` <span class="badge-verified">verified</span> | Returns every tenant from the root down to and including the direct parent |
| `getDescendants(id)` <span class="badge-verified">verified</span> | Returns the **active-scoped** subtree: active descendants only (uses `ltree` for efficiency) |
...
Methods marked <span class="badge-verified">verified</span> have a documented, behaviorally-guaranteed result scope: the exact set they return is part of the API contract, not incidental to the query.

<Aside type="caution" title="getDescendants is active-scoped">
  `getDescendants` lists the **active-scoped** subtree: active descendants only.
  Archived and soft-deleted tenants are excluded, so the result is not
  necessarily every row that sits beneath the tenant in the tree. Likewise
  `getAncestors` now includes the direct parent. If you need inactive tenants
  too, query them explicitly rather than assuming the traversal returns them.
</Aside>
