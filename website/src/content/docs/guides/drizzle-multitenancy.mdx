...
description: Add automatic tenant isolation to Drizzle ORM using PostgreSQL Row-Level Security and Stratum's drizzleWithTenant wrapper.
---
...
Stratum's `drizzleWithTenant()` wrapper for Drizzle takes a different approach. It wraps every `execute()` and `transaction()` call so that PostgreSQL's Row-Level Security handles tenant isolation at the database level. No manual `.where()` filters needed.
...
const tenantDb = drizzleWithTenant(db, () => currentTenantId, pool);
...
<Aside type="note">
  The `drizzleWithTenant` function was previously named `drizzleWithTenantScope`.
  The old name still works as a deprecated alias, but new code should use
  `drizzleWithTenant` for consistency with the Prisma adapter (`prismaWithTenant`).
</Aside>
...
app.use(async (req, res, next) => {
  ...
  req.tenantDb = drizzleWithTenant(db, () => tenantId, pool);
...
```typescript
const tenantDb = drizzleWithTenant(db, () => "", pool);
