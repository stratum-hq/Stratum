...
Stratum does not enforce a maximum tree depth; tenants can nest as deeply as your data requires. Ancestry and subtree queries walk the full `ancestry_ltree` path, so very deep hierarchies cost more per query; keep nesting reasonable for your workload.
